...
Describe 'Export-PayloadToFile' {
    It 'should demand and only accept XML object' {
        Get-Command "$commandName" | Should -HaveParameter Payload -Type xml
        Get-Command "$commandName" | Should -HaveParameter Payload -Mandatory
    }
}